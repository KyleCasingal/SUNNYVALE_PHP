...
if ($_SESSION['subdivision'] != '') {
  $result = $con->query("SELECT * FROM post, user WHERE user.full_name = post.full_name AND officer_post = 'No' AND post_status = 'Active' AND subdivision = '" . $_SESSION['subdivision'] . "' ORDER BY post_id DESC") or die($mysqli->error);
  ...
  $result = $con->query("SELECT * FROM post, user WHERE user.full_name = post.full_name AND officer_post = 'No' AND post_status = 'Active' ORDER BY post_id DESC") or die($mysqli->error);
